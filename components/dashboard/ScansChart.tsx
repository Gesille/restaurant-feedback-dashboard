...
import { useGetScanTrendQuery } from "@/redux/dashboard/dashboardApi";

const ACCENT = "#6C4DF4";

export function ScansChart() {
  const { data, isLoading } = useGetScanTrendQuery();
  const scanTrend = data?.data ?? [];

  if (isLoading || scanTrend.length === 0) {
    return (
      <Card className="p-6 font-mono-custom">
        <div className="h-64 animate-pulse rounded-xl bg-slate-100" />
      </Card>
    );
  }

  const peak = scanTrend.reduce((max, d) => (d.scans > max.scans ? d : max), scanTrend[0]);
    ...
    <Card className="p-6 font-mono-custom">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <p className="text-lg font-semibold tabular-nums text-slate-900">{total.toLocaleString()}</p>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={scanTrend} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            <Area
              ...
              fill="url(#scansFill)"
              dot={(props: any) => {
                if (props.payload.date !== peak.date) return <></>;
                return <circle key={`peak-${props.payload.date}`} cx={props.cx} cy={props.cy} r={4} fill={ACCENT} stroke="#fff" strokeWidth={2} />;
              }}
...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div style={{ borderRadius: 12, border: "1px solid #F1F5F9", boxShadow: "0 8px 24px -12px rgba(15,23,42,0.15)", background: "#fff", padding: "8px 12px" }}>
      <p className="text-[11px] font-medium text-slate-400">{label}</p>
      <p className="text-sm font-semibold text-slate-900 tabular-nums">{payload[0].value.toLocaleString()} scans</p>
    </div>
  );
}